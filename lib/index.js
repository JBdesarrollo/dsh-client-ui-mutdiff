 ...
 * ask for the behaviour (`DSH_MUTDIFF_AUTO_OPEN=1 dsh web`), need no file, and
 * the plugin never writes one: it has no filesystem write call at all.
 *
 * The naming follows the harness's own invocation-time capability variables
 * (`$DSH_WEB_SEARCH_PROVIDER`, `$BROWSER`): a value present for this process
 * wins over the row's config, because naming the variable *is* the deliberate
 * act, while a config file may have been written months ago.
 */
export const ENV_EDITOR = 'DSH_MUTDIFF_EDITOR'
export const ENV_AUTO_OPEN = 'DSH_MUTDIFF_AUTO_OPEN'
export const ENV_GOTO_LINE = 'DSH_MUTDIFF_GOTO_LINE'
 ...
 *
 * 1. The environment — `DSH_MUTDIFF_AUTO_OPEN=1`, `DSH_MUTDIFF_EDITOR=code`,
 *    `DSH_MUTDIFF_GOTO_LINE=0` — which applies to one invocation and touches no
 *    file.
