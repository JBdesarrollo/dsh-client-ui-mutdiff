 ...
 *
 * Both triggers funnel into one reveal, and the reveal **publishes** the
 * location on the activity feed (`GET /mutdiff/activity`) for whoever is
 * connected — an editor extension that can move a caret without raising a
 * window, this plugin's own page, anything on loopback. The editor's CLI is a
 * separate, opt-in step (`MUTDIFF_FOLLOW=cli`) taken only while *nothing* is
 * connected, because `code -r -g` brings its window to the front by design and
 * no flag stops it: interrupting a reader is a choice they make, not a default
 * this plugin picks. A burst of mutations to one file coalesces into a single
 * published location, and a file already sitting on that line is not
 * re-published, so a long edit loop arrives as a trail of edits rather than a
 * storm.
 *
...
const ACTIVITY_PATH = `${ROUTE_PREFIX}/activity`
/** How often an idle activity stream is kept alive, in milliseconds. */
...
export function normalizeConfig(raw, env = process.env) {
  ...
  const rowFollowCli = source.followCli === true ? true : undefined
  /*
  ...
  /*
   * Follow, and the one word that lets it raise a window.
   *
   * `MUTDIFF_FOLLOW=1` publishes where the agent is working and nothing else —
   * an editor connected to that feed reveals it without being asked and without
   * stealing focus. The CLI is reached only under the `cli` word, because
   * `code -r -g` brings its window to the front by design and there is no flag
   * to stop it: that is an intrusion a reader has to choose, not a default this
   * plugin gets to pick for them.
   */
  const followValue = variable(ENV_FOLLOW)
  const followOff = followValue !== undefined && FALSEY.test(followValue)
  const followCliWord = followValue !== undefined && /^(cli|raise|window)$/i.test(followValue)
  const followOn = followValue === undefined ? undefined : !followOff
  const rowFollowOn = rowFollow ?? rowFollowCli
  const follow = followOn ?? rowFollowOn ?? false
  const followCli = (followCliWord ? true : undefined) ?? rowFollowCli ?? false
  return {
    ...
    autoOpen,
    /**
     * Follow the agent's edits from the host, in real time, and publish each one
     * to whoever is connected. Off by default; it opens no window by itself.
     */
    follow,
    /**
     * Whether follow may fall back to the editor's CLI — which raises the editor
     * window — when no editor is connected. Off by default, and only ever set by
     * asking for it (`MUTDIFF_FOLLOW=cli`).
     */
    followCli,
    ...
    sources: {
      ...
      follow: followOn !== undefined ? 'env' : rowFollowOn !== undefined ? 'config' : 'default',
      followCli: followCliWord ? 'env' : rowFollowCli !== undefined ? 'config' : 'default',
...
export function createFollower(options = {}) {
  ...
  /**
   * Where the resolved location goes. Defaults to nothing, so a follower built
   * without a feed still answers "where is the agent working" through its
   * subscriber count and spawns nothing.
   */
  const publish = typeof options.publish === 'function' ? options.publish : () => {}
  /**
   * Whether an editor is connected and doing its own revealing. While one is,
   * the CLI is not reached even in `followCli` mode: two reveals would fight
   * over the same caret, and the connected one is the one that does not raise a
   * window.
   */
  const hasEditor = typeof options.hasEditor === 'function' ? options.hasEditor : () => false
  ...
  const reveal = (file, line) => {
    ...
    const at = now()
    // Publishing is unconditional: where the agent is working is true whether or
    // not anyone is listening, and it is the subscriber's business what to do
    // with it. Raising a *window*, by contrast, is an intrusion, so the CLI is
    // reached only when the reader asked for it and no editor is connected to do
    // it properly.
    let published = true
    try {
      publish({ path: file, line, time: at })
    } catch {
      published = false
    }
    let landed = published
    if (config.followCli && !hasEditor()) {
      let result
      try {
      ...
      landed = result?.ok === true
    }
    if (landed) revealed.set(file, { line, at })
  }
...
/**
 * The activity feed: who is connected to hear where the agent is working, and
 * what the last thing it said was.
 *
 * This is the half of live follow that makes the feature usable without ever
 * raising a window. The host resolves the line exactly (see
 * {@link createFollower}) and hands it to whoever is connected — a VS Code
 * extension that can reveal it with `preserveFocus`, the harness page itself,
 * anything on loopback that speaks Server-Sent Events — and only reaches for the
 * editor's CLI when nobody is connected *and* the reader asked for that
 * fallback.
 *
 * Subscribers carry a `role`, and the distinction is load-bearing rather than
 * cosmetic: `editor` means "I will reveal this myself, do not spawn a CLI",
 * while `ui` means "I am only drawing it". Without it, opening the harness page
 * would silently disable the editor fallback for everyone.
 *
 * @param options - a clock seam (tests drive the heartbeat deterministically).
 * @returns `{handler, push, count, size, subscriptions, last, close}`.
 */
export function createActivityFeed(options = {}) {
  const now = typeof options.now === 'function' ? options.now : () => Date.now()
  const heartbeatMs = options.heartbeatMs ?? ACTIVITY_HEARTBEAT_MS
  /** Connected subscribers, each with the response it writes to and its role. */
  const subscribers = new Set()
  let last = null

  const roleOf = (url) => {
    try {
      const value = new URL(url ?? '/', 'http://localhost').searchParams.get('role')
      return value === 'ui' ? 'ui' : 'editor'
    } catch {
      return 'editor'
    }
  }

  const write = (subscriber, event, data) => {
    try {
  ...
  const push = (activity) => {
    ...
    for (const subscriber of [...subscribers]) {
      // Only the editor role is handed a caret target; a `ui` subscriber gets the
      // same payload, and decides on its own to draw it.
      write(subscriber, 'activity', activity)
    }
  }
  ...
  const handler = (req, res) => {
    ...
    // A subscriber that reconnects is told how long to wait, and where things
    // stand right now, so a fresh window is not blank until the next edit.
    res.write('retry: 2000\n\n')
    const subscriber = { res, role: roleOf(req?.url), at: now() }
    subscribers.add(subscriber)
    write(subscriber, 'hello', { ok: true, last, subscribers: subscribers.size })
    const close = () => { subscribers.delete(subscriber) }
    req.on?.('close', close)
    res.on?.('close', close)
  }

  const timer = typeof options.timer === 'object' ? options.timer : { set: (fn, ms) => setInterval(fn, ms), clear: (handle) => { clearInterval(handle) } }

  return {
    handler,
    push,
    /** Subscribers, optionally of one role only. */
    count(role) {
      if (role === undefined) return subscribers.size
      let total = 0
      for (const subscriber of subscribers) if (subscriber.role === role) total += 1
      return total
    },
    size() {
    ...
    subscriptions() {
      return [...subscribers].map((subscriber) => subscriber.role)
    },
    lastSeen() {
      return last
    },
    /** Keep the streams alive through an idle spell; unref'd so it never holds the process open. */
    start() {
      const handle = timer.set(() => {
        for (const subscriber of [...subscribers]) write(subscriber, 'ping', { at: now() })
      }, heartbeatMs)
      handle?.unref?.()
      return handle
    },
...
export function createBridge(options = {}) {
  ...
  const activity = options.activity ?? createActivityFeed()
  ...
  const state = () => {
    ...
    return {
      ...
      followCli: config.followCli,
      // Who is listening to the activity feed. An `editor` subscriber reveals
      // without raising a window, which is why its presence is reported: it is
      // what the CLI fallback waits for.
      subscribers: activity.count(),
      editorSubscribers: activity.count('editor'),
  ...
  return { handler, state, open, config, activity }
}
...
export function apply(ctx, rawConfig) {
  ...
  if (config.follow && typeof ctx.on === 'function') {
    const follower = createFollower({
      config,
      open: bridge.open,
      publish: (activity) => { bridge.activity.push(activity) },
      hasEditor: () => bridge.activity.count('editor') > 0,
    })
    ...
    ctx.logger?.info?.(
      config.followCli
        ? `[dsh-client-ui-mutdiff] live follow on (${ENV_FOLLOW}): edits are published, and the CLI reveals them only while no editor is connected`
        : `[dsh-client-ui-mutdiff] live follow on (${ENV_FOLLOW}): edits are published to every connected editor; no window is ever raised (use ${ENV_FOLLOW}=cli to allow the CLI fallback)`
    )
  }
